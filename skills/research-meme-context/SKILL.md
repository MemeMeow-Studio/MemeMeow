...
3. 先补齐未知的检索锚点。
   ...
   当角色、模板、出处或外部引用的名称未知，或图片文字过于普通时，优先对整图进行 `reverse_image`。它的价值是从像素中发现可用于后续文字检索的名称、别名、传播页和相似变体，而不是直接判定梗义或出处。当前 Agent 语境对反向图片 provider 只有一次调用机会，因此首次应选择最能回答当前证据问题的有效图片；不要预留或计划第二次 provider 调用。
