...
- Executor 使用当前 attempt 的短期 capability 从模型 broker 读取累计金额，达到终止限额后复用现有进程组终止和回收流程；允许检查间隔内已完成调用造成金额超过限额。
...
- Executor：通过模型 broker 读取绑定 attempt 的累计金额，达到终止限额时复用现有进程组终止和回收流程；OpenCode 数据库改为任务专属路径。
