...
- 本期不增加登录、用户表或角色系统；Agent 到 API 的服务认证和任务执行绑定由 `secure-agent-internal-callbacks` change 统一提供。
...
`reverse_image_policy` 使用字符串枚举 `forbid | auto`。上传表单、单图 JSON、图片库一键处理和重试请求都显式携带它；批量请求在顶层选择一次并应用到本批次各项。入口只提供受校验的业务选项，图片处理服务把规范化值冻结到 job revision，`ImageProcessingWorker` 创建 `meme_context_generation` Task 时再复制到可信任务输入；客户端不直接创建或覆盖叶子 Task payload。历史 job/Task 缺失策略时按 `forbid`，Worker 不从当前前端状态或全局环境推断。

缺省值选择 `forbid`，既兼容历史任务，也避免部署方配置了供应商密钥后无意扩大图片外发范围。提交 `auto` 时后端先检查供应商是否配置；未配置直接拒绝，避免“允许但永远不可用”的静默降级。

活动 job/Task 仍以 scope、Meme、目标图片 SHA 和单图片活动唯一约束互斥。去重查询同时比较目标 SHA、Agent/处理配置指纹和策略：全部相同才复用，策略或配置不同返回 `generation_policy_conflict`，不得并行覆盖同一份图片语境。没有选择把策略简单加入允许并行的唯一键，因为那会允许两个策略同时处理并覆盖结果。

### 2. 使用受当前 Task 执行权保护的内部接口，不把供应商密钥交给 Agent

新增供应商无关的 `POST /internal/reverse-image/search`，放在现有 FastAPI 服务中并纳入 `agent-internal-callbacks` 的统一 fail-closed 边界。接口必须在读取 multipart 和查询 Task 前验证服务身份，再验证绑定当前 `task_id`、scope、claim generation/owner、attempt、允许 operation、目标 SHA 和有效期的执行凭据。请求业务字段使用 multipart：

- `task_id`
- `image`（必须匹配任务目标整图；裁剪改由后端从受控源图按受限参数生成）
- `search_type`
- `language`
- 可选 `country`、`query`、`auto_crop`、`refresh`

服务认证通过后，接口通过受验证的 `task_id` 查询数据库，只接受处于 `running`、claim generation 非零、owner/generation 匹配且租约未过期的 `meme_context_generation`，并从任务 payload 读取 scope、目标 SHA 和策略。调用方不提交 scope 或 policy；请求图片必须匹配目标 SHA，裁剪由后端绑定源图和当前执行生成，不能借用一个活动任务检索任意图片。

Runner 给 Agent 的是内部接口地址、任务标识和仅绑定当前 Task/claim/attempt/目标/允许 operation 的短期 callback 执行凭据。根 secret、API 到 executor 的 Bearer token 和 `SERPAPI_API_KEY` 都不进入 Agent。`forbid` 同时写入 prompt 约束；真正的标准能力边界由内部接口再次检查服务凭据、当前 Task 执行权和任务记录。

备选方案是继续由脚本直连供应商并在结束后上报事件。它改动较少，但密钥仍暴露给 Agent，直接 HTTP 调用可以绕过缓存和计数，因此不采用。只使用静态服务 token 也不足以阻止同一 Agent 借用其它活动 task id，因此服务认证必须叠加当前 Task claim 的执行绑定。

### 3. Skill 脚本缩减为内部接口的薄 CLI 客户端

保留现有命令名和搜索参数，脚本改为：解析参数、读取内部接口地址、任务标识与任务级 callback 凭据、提交目标整图或受限裁剪参数、打印统一 JSON、把稳定错误写到 stderr。脚本删除 `.env`/`SERPAPI_API_KEY` 读取、任意图片上传旁路、供应商 URL、共享缓存写入和供应商响应脱敏职责。
...
- [Risk] callback 凭据泄露后被用于其它任务或旧 claim。→ 凭据只绑定单 Task/claim/attempt/operation/目标且短期有效；每次调用仍与数据库当前 claim 比较，认证失败在读取图片与 Task 前收束。
...
1. 先新增 usage events 表、索引和只向前迁移，部署后端服务，但不开放无认证 callback，也不移除脚本直连路径。
2. 完成 `make-application-scope-aware` 的完整 claim fencing 与 factory scope 一致性实现，再实施 `secure-agent-internal-callbacks` 的 verifier、执行绑定和认证前 body 拒绝。
3. 更新 API 请求、任务 payload、去重规则、Runner prompt 与内部接口连接信息；历史缺失策略按 `forbid`。
4. 切换带任务级 callback 凭据的薄客户端和 Skill 文档，删除供应商直连路径，确认 Agent 环境不含根 secret、executor token 或 SerpApi 密钥。
5. 更新前端四类入口和任务审计展示，再启用 `auto` 选择；基础 capability 归档后再归档 callback 安全收紧。
6. 回滚应用代码时保留新增表并关闭 callback 和新的 Agent 调度；不得恢复无认证接口或重新把供应商密钥交给 Agent。
