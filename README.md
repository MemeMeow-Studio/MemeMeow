...
- **受控并发**：Agent lane 使用 PostgreSQL 持久公平 claim；全局并发由 `MEMEMEOW_OPENCODE_CONCURRENCY` 控制，单个 scope 的同时运行数由 `MEMEMEOW_AGENT_SCOPE_CONCURRENCY` 控制，任务的 `lane_resource_key` 还可绑定独立资源槽位。所有值只要求正整数和必要的关系校验，由部署环境决定容量；槽位满时任务保持排队，队列不设置有限数量门禁。缓存和 metadata repair 保留独立资源。
