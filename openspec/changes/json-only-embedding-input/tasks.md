...
- [x] 4.5 将持久任务服务和 Agent worker 接入应用启动/关闭生命周期，为上传不阻塞、缺失 OpenCode、重复任务、批量部分失败、重启查询、异步自动命名和人工字段保护补充 API 测试。
...
- [x] 5.4 为筛选、分页、活跃刷新、终态停止轮询、上传跳转、失败重试、敏感字段隐藏和桌面/移动布局补充前端测试。
...
- [x] 7.2 在测试 runtime 中完成至少一张图片从 `pending -> Agent task -> ready sidecar -> v4 embedding` 的端到端验收，并确认 Agent 无法直接修改 canonical sidecar 或应用代码。
