...
生产 Agent 镜像固定使用 OpenCode `1.18.18`。OpenCode 在任务专属 SQLite 中保存 session 累计 `cost`，插件能通过当前 OpenCode 进程提供的 SDK 读取主 session 已完成请求的用量。生产 Executor 通过模型 broker 查询绑定 attempt 的累计金额，负责检查取消和总超时，并能终止及回收整个进程组。

提醒使用 OpenCode 记录的主 session 累计金额；生产终止使用模型 broker 保存的 attempt 累计金额。现有轮次字段继续服务页面活动观察，不进入提醒或终止判断。
...
### 4. Executor 从模型 broker 读取累计金额并终止进程组

生产 Executor 使用当前模型 capability 请求部署固定 broker 的 `/analysis-usage` 接口，并通过 `X-MemeMeow-Executor-Attempt-ID` 绑定当前 attempt。broker 根据已经完成的模型调用返回 `executor_attempt_id`、`observed_cost` 和带时区的 `checked_at`。恢复时，新的 broker attempt 以持久化的最近可信金额为起点；Executor 同时把该金额作为本地单调下限。

查询禁止 HTTP 跳转和系统代理，只接受固定三个字段，并限制响应大小。attempt 冲突、金额倒退、无效金额、无时区时间和网络故障均产生 `agent_analysis_usage_unavailable`。本地开发兼容路径可以读取任务 SQLite；Server 启用策略时必须同时提供 broker endpoint 和当前 attempt capability。
...
新 attempt 可以再次发送一次提醒。Server 为新 capability 设置之前保存的最近可信金额，broker 在此基础上累计新调用金额。旧 Worker 的迟到状态受到 claim generation 和 attempt 绑定约束，不能覆盖新 attempt 的金额、提醒状态或终态。
...
- OpenCode 本地 `cost` 只决定提醒；模型 broker 的 attempt 累计金额决定生产终止。供应商最终账单仍由现有计费系统结算。
- 插件状态文件与 Agent 使用同一 Unix 身份，任务可能影响提醒状态。提醒属于收尾提示；broker 金额与 Executor 终止流程不读取该状态。
